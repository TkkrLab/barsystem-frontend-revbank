...
class RpcClient:
	...
	def productBarcode(self, barcode, type=None):
		if type == None:
			...
			return self._request("product/barcode", {"barcode": barcode, "type": type})
		
	def productBarcodeTypes(self):
		return self._request("product/barcode/types")
	
	def productLocation(self, location=None):
		return self._request("product/location", location)
	...
	def getGroups(self, query=None):
		return self._request("person/groups", query)
			
	# OLD STUFF
	
	#def blame(self):
	#	return []
	#	return self._request("person/blame", [-13.37])
